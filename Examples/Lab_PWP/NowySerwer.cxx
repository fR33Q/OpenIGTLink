...
int ReceivePoint(igtl::Socket* socket, igtl::MessageHeader* header)
{
	...
	igtl::PointMessage::Pointer pointMsgToSend;
	pointMsgToSend = igtl::PointMessage::New();
	pointMsgToSend->SetDeviceName("PointSender");
	...
	if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
	{
		...
		for (int i = 0; i < nElements; i++)
		{
			...
			negpoint->SetPosition(newpos);
			negpoint->SetRadius(pointElement->GetRadius());
			negpoint->SetOwner(pointElement->GetOwner());


			pointMsgToSend->AddPointElement(negpoint);
		}
		igtl::Sleep(10000);
		pointMsgToSend->Pack();
		socket->Send(pointMsgToSend->GetPackPointer(), pointMsgToSend->GetPackSize());
	}
